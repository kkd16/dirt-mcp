...
function compactView(view: ScanOrthographicViewBlocksOutput): z.infer<typeof ScanOrthographicViewGridOutputSchema> {
  ...
  for (const block of view.blocks) {
    ...
    if (blockStateIndexRow === undefined || distanceRow === undefined || columnIndex < 0 || columnIndex >= width) {
...
function errorResult(error: unknown, context: string): CallToolResult {
  const detail =
    error instanceof DirtToolError
      ? error
...
export function registerTools(server: McpServer, config: BridgeConfig, registrations: RegisteredTool[]): void {
  ...
  registrations.push(
    register(
      ...
      async (_input, context) =>
        auditToolCall('ping_server', undefined, context, async (callId) => {
          try {
  ...
  registrations.push(
    register(
      ...
      async (_input, context) =>
        auditToolCall('get_server_status', undefined, context, async (callId) => {
          try {
  ...
  registrations.push(
    register(
      ...
      async (input, context) =>
        auditToolCall('count_region_block_states', input.world, context, async (callId) => {
          try {
  ...
  registrations.push(
    register(
      ...
      async (input, context) =>
        auditToolCall('get_region_blocks', input.world, context, async (callId) => {
          try {
            const result = await bridgeRequest(
              config,
  ...
  registrations.push(
    register(
      ...
      async (input, context) =>
        auditToolCall('scan_orthographic_view', input.world, context, async (callId) => {
          try {
            const { format, ...bridgeInput } = input;
            const sparseView = await bridgeRequest(
              config,
              '/v1/scan-orthographic-view',
              callId,
              ScanOrthographicViewBlocksOutputSchema,
              jsonPost(bridgeInput),
              30_000,
            );
            if (format === 'grid') {
  ...
  registrations.push(
    register(
      ...
      async (input, context) =>
        auditToolCall('replace_region_blocks', input.world, context, async (callId) => {
          try {
            const result = await bridgeRequest(
              config,
  ...
  registrations.push(
    register(
      ...
      async (input, context) =>
        auditToolCall('fill_region', input.world, context, async (callId) => {
          try {
            const result = await bridgeRequest(
              config,
              '/v1/fill-region',
  ...
  registrations.push(
    register(
      ...
      async (input, context) =>
        auditToolCall('set_blocks', input.world, context, async (callId) => {
          try {
            const result = await bridgeRequest(
              config,
              '/v1/set-blocks',
  ...
  registrations.push(
    register(
      ...
      async (input, context) =>
        auditToolCall('undo_last_dirt_edit', input.world, context, async (callId) => {
          try {
            const result = await bridgeRequest(
              config,
  ...
  registrations.push(
    register(
      ...
      async (input, context) =>
        auditToolCall('run_minecraft_commands', undefined, context, async (callId) => {
          try {
            const result = await bridgeRequest(
              config,
