...
test('sends an authenticated exact bridge request through the injected fetch function', async () => {
  ...
  const result = await client.request(BRIDGE_ROUTES.countRegionBlockStates, 'call-id', ResponseSchema, {
    world: 'world',
  });
  ...
  assert.equal(headers.get('X-Dirt-Call-Id'), 'call-id');
});
...
test('does not send a body or content type for GET requests', async () => {
  ...
  await client.request(BRIDGE_ROUTES.ping, 'call-id', ResponseSchema);
...
test('preserves structured bridge failures and classifies network failures', async () => {
  const structured = new BridgeClient({ origin: 'http://127.0.0.1', token: 'token' }, async () =>
    Response.json({ error: { code: 'world_busy', message: 'World is busy' } }, { status: 409 }),
  );
  await assert.rejects(
    structured.request(BRIDGE_ROUTES.ping, 'call-id', ResponseSchema),
    (error: unknown) =>
      error instanceof ToolFailure && error.code === 'world_busy' && error.message === 'World is busy',
  );
  ...
  await assert.rejects(
    unavailable.request(BRIDGE_ROUTES.ping, 'call-id', ResponseSchema),
    (error: unknown) =>
